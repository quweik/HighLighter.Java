...
public class MakeButton extends JButton implements MouseListener {
    ...
    public Dimension getPreferredSize() {
       // this.setFont(new Font("宋体",Font.PLAIN,16));
        String text = getText();
        FontMetrics fm = this.getFontMetrics(getFont());// 获取指定字体的规格
    ...
    public void paintComponent(Graphics g) {
        ...
        drawLiquidButton(new Color(89,210,220,0), this.getWidth(), this
    ...
    protected void drawDropShadow(int w, int h, float scale, Graphics2D g2) {
        g2.setColor(new Color(89, 210, 220));
    ...
    protected void drawButtonBody(int w, int h, float scale, Color base,
        ...
        this.fillRoundRect(g2,
                ...
                scale * (.4f), scale * (.4f), w - scale * .8f, h - scale * .5f,
                scale * .6f, scale * .4f);
    }
    ...
    protected static void drawRoundRect(Graphics2D g2, float x, float y,
                                        float w, float h, float ax, float ay) {
        g2.drawRoundRect((int) x, (int) y, (int) w, (int) h, (int) ax,
                (int) ay);
    }
